    ...
    <transition name="fade">
      <div v-if="menuOpen" class="fixed inset-0 z-[999] flex">
        ...
        <transition name="slide-scale">
          ...
          >
            <div class="relative h-full px-4 sm:px-6 py-8 sm:py-10">
              <!-- Close button -->
              <button @click="toggleMenu" class="absolute top-7 right-4 sm:right-6 z-[1000] hover:scale-110 transition duration-300">
              ...
              <transition name="panel-content">
                <div class="flex flex-col justify-between h-full pt-10 sm:pt-6">
                  <div class="flex flex-col md:flex-row gap-8 md:gap-10 h-2/3 justify-center items-start md:items-center">

                    <!-- Left: Social/Color/Contact -->
                    <div 
                      class="space-y-6 md:space-y-8 text-sm w-full md:w-1/2 pl-2 sm:pl-6 lg:pl-10 xl:pl-36" 
                      :class="isDark ? 'text-gray-300' : 'text-gray-800'"
                    >
                      <div :class="['panel-item', 'delay-100', { 'entered': panelContentVisible }]">
                        <h4 
                          class="font-semibold text-xs mb-2 border-b pb-1 w-[50%]" 
                      ...
                      <div :class="['panel-item', 'delay-200', { 'entered': panelContentVisible }]">
                        ...
                        >COLOR</h4>
                        <p 
                          class="hover:underline cursor-pointer font-medium transition-transform duration-300 ease-in-out flex items-center gap-2" 
                    ...
                    <div :class="['panel-item', 'delay-400', { 'entered': panelContentVisible }]" class="w-full md:w-1/2">
                      <ul 
                        class="space-y-3 sm:space-y-5 text-3xl sm:text-4xl font-medium w-full sm:w-3/4 md:w-2/3"
                  ...
                  <!-- Bottom Image -->
                  <div :class="['panel-item', 'delay-500', { 'entered': panelContentVisible }]" class="mt-4 sm:mt-6 h-1/3 w-full">
                    <img
                      src="https://pbs.twimg.com/media/GpdXqUCXcAA2f8d?format=jpg&name=large"
                      alt="Photographer"
                      class="rounded-lg shadow-md object-cover w-full h-36 sm:h-48 md:h-64 lg:h-auto"
                    />
                  </div>
                </div>
              </transition>
...
// Récupérer les fonctions du thème
...
// Fermeture du menu lors du changement de route sur mobile
...
// Écouter le changement de route pour fermer le menu
onMounted(() => {
  router.afterEach(() => {
    if (menuOpen.value) {
      toggleMenu()
